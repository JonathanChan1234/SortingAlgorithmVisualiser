...
const SortingCompetition: React.FC = () => {
	...
	const initSortElements = (number: number) => {
		console.log(`Init Sort Elements: ${number}`);
		console.log(`-----------------`);
	...
	const renderSortingModel = () => {
		console.log(
			`render sorting element (number of element: ${throttleItem})`
		);
	...
	const startAnimation = async (sortingResult: SortingResult[]) => {
		const maxIteration = Math.max(
			...sortingResult.map(result => result.immediateResult.length)
		);
		let newSortArray: SortElements = {};
		for (let i = 0; i < maxIteration; ++i) {
			for (const sortingResultByAlgorithm of sortingResult) {
				const immediateResultLength =
					sortingResultByAlgorithm.immediateResult.length;
				...
				if (i < immediateResultLength - 1) {
					newSortArray = {
						...
						[sortingResultByAlgorithm.algorithm]: {
							comparison: i,
							sortArray:
								sortingResultByAlgorithm.immediateResult[i],
